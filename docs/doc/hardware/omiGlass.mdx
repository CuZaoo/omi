...
The web app opens directly into the Optical Lab workspace. The device connection is a header control, so camera history,
model settings, diagnostics, and restored sessions remain available while the glasses are disconnected.

- **Optical feed** shows the selected frame and controls single or interval capture from 5 to 300 seconds.
- **Pipeline** tracks BLE transfer, frame assembly, local persistence, vision analysis, and reasoning for every frame.
- **AI assistant** answers questions about the selected frame. Manual analysis is the default; automatic analysis can be enabled per browser.
- **Advanced controls** contain camera tuning, provider/model settings, device information, connection tests, and sanitized logs.
- **Local WiFi scan** uses the Windows development host to list nearby networks and highlight the computer's current SSID without requiring a BLE scan command. The glasses still require the photo-control characteristic to receive credentials.

<Note>
  ESP32-S3 supports 2.4 GHz WiFi only. The console marks 5 GHz-only networks as unsupported. Browsers cannot read WiFi
  SSIDs directly, so local scanning is exposed only by the Expo development server and is not available in a static public build.
</Note>

### Camera controls

The advanced drawer exposes every OV2640 option supported by the firmware: resolution, JPEG quality, brightness,
contrast, saturation, effects, white balance, exposure, gain, corrections, mirroring, flipping, and the test color bar.
Settings are saved in `localStorage` and can be reapplied after reconnecting.

For a green cast under fluorescent or LED lighting, use **Office anti-green** first. It enables automatic white balance,
selects office white balance mode, and lowers saturation by one step. Sunny, cloudy, office, and home modes can also be
selected manually. Sharpness and denoise are not shown because the OV2640 driver reports them as unsupported.

The browser restores an already-authorized OMI Glass with `navigator.bluetooth.getDevices()` and retries unexpected
disconnects after 1, 2, and 4 seconds. The latest 10 sessions are stored in IndexedDB with up to 200 frames each.

<Warning>
  `EXPO_PUBLIC_OPENAI_API_KEY` and `EXPO_PUBLIC_GROQ_API_KEY` are embedded in the browser bundle. Cloud providers in this
  console are for local development only. Do not publicly deploy a build containing these keys.
</Warning>
...
| Mode | Flow |
| --- | --- |
| Staged | OMI Glass camera → BLE frame → Ollama or OpenAI vision description → Groq, OpenAI, or Ollama reasoning |
| Direct | OMI Glass camera → BLE frame → OpenAI Responses API multimodal input |

OpenAI image requests use the Responses API with `input_image`. Ollama uses its `/api/chat` endpoint with a base64 image.

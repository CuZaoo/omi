...
export const DeviceView = React.memo(({ deviceController }: { deviceController: DeviceController }) => {
    ...
    const liveStreamResMap: Record<string, { framesize: number; quality: number; intervalMs: number }> = {
        ...
        '均衡': { framesize: 5, quality: 15, intervalMs: 1500 },
        '清晰': { framesize: 8, quality: 12, intervalMs: 2500 },
    };

    const handleToggleLiveStream = React.useCallback((active: boolean) => {
        setLiveStreamActive(active);
        const params = liveStreamResMap[liveStreamRes];
        glass.toggleLiveStream(active, params.framesize, params.quality, params.intervalMs);
    }, [glass, liveStreamRes]);

    const handleLiveStreamRes = React.useCallback((res: '流畅' | '均衡' | '清晰') => {
        setLiveStreamRes(res);
        if (liveStreamActive) {
            const params = liveStreamResMap[res];
            glass.toggleLiveStream(true, params.framesize, params.quality, params.intervalMs);
        }
    }, [glass, liveStreamActive]);
            ...
            <View style={styles.captureSection}>
                ...
                <View style={styles.liveViewRow}>
                    <View style={styles.liveViewToggle}>
                        <Text style={styles.liveViewLabel}>Live View</Text>
                        <Switch
                    ...
                    {(['流畅', '均衡', '清晰'] as const).map(res => (
                        <Pressable key={res} onPress={() => handleLiveStreamRes(res)} style={[styles.liveResButton, liveStreamRes === res && styles.liveResActive]}>
                            <Text style={[styles.liveResText, liveStreamRes === res && styles.liveResTextActive]}>{res}</Text>
                        </Pressable>
                    ))}
                </View>
